...
- `stock_team`：行情与账户数据、计算、研究工具、Stage 0/Stage 1 证据包、盘中事实和报告。
...
恢复/开始当日
-> 确认账户、持仓、近期交易和认知约束
-> stock_team 生成 Stage 0 市场事实
-> 用户 + 高阶模型通过指定 skill 讨论并形成今日计划
-> stock_team 只对确认后的关注池生成 Stage 1 证据
-> 高阶模型确认权限、风险和盘中条件
-> 盘中观察/记录
-> 盘后复盘
-> 用户确认后才吸收经验与教训
...
- 不得跳过 `stock_team` 或用模型臆测代替 Stage 0。
- Stage 0 必须是明确时点的真实盘前数据；昨日收盘数据不得伪装成盘前数据。
- Stage 1 必须晚于 Stage 0 讨论，且只能覆盖用户确认的关注池。
- Dashboard 盘中不得自动重算或改写 Stage 0、Stage 1。
...
1. 高阶模型审计现有脏工作区，按功能和风险拆分可保留改动。
2. 固化任务单、执行日志和审核记录模板。
3. 配置 Git remote；之后严格执行“一任务一 commit，审核后 push”。
4. 验收基础协调器是否能驱动真实固定产物，而不只是展示五个静态步骤。
5. 用一次非交易模拟验证：恢复会话 -> Stage 0 -> skill 讨论 -> Stage 1 -> 盘中 -> 收尾。
6. 基础链稳定后，再逐步完善 Agent、`stock_team` 能力和界面。
