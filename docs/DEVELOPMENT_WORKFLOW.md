...
1. Stabilize the current dashboard changes first.
   Goal: make the existing `dashboard_server.py` + `dashboard_writer.py` + template changes reliably render and run.
2. Only after the dashboard is stable, continue dashboard feature work.
   Goal: finish missing planned pieces such as node history or JSON readability improvements.
3. Then move to report enrichment.
   Goal: fix thin research output by preserving per-symbol agent outputs and richer memo/report content.
4. Defer broad refactors until a user-visible pain point forces them.
