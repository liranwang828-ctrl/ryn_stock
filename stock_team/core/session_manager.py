...
def handle_text(text: str, date_str=None):
    ...
    # ── 优先：LLM意图识别层 ───────────────────────────────────
    try:
        from stock_team.utils.translator import handle as _translate
...
def run_node(node: int, symbols=None, date_str=None) -> str:
    ...
    if node == 0:
        # 快速健康检查（只显示警告）
        try:
            from stock_team.utils.health_check import run as _hc
        ...
        if pos_syms:
            try:
                from stock_team.core.premarket_exit_sheet import (
        ...
        lines.append(f"\n【Step 6.5】盘前操作清单")
        try:
            from stock_team.core.premarket_checklist import generate_checklist as _gen_cl
        ...
        if cio_syms and _is_monday:
            ...
            try:
                from stock_team.core.cio import run_parallel as _cio_parallel
        ...
        # 模拟盘强制收盘平仓
        try:
            from stock_team.core.paper_trader import eod_close_all, ACCOUNTS
