...
TACTICAL_RULES_PATH = os.path.join(BASE, "config", "tactical_rules.json")
...
def _load_acct_cfg(acct):
    if os.path.exists(TACTICAL_RULES_PATH):
        try:
            data = json.load(open(TACTICAL_RULES_PATH, encoding="utf-8"))
            return data.get("accounts", {}).get(acct, {})
        except Exception:
            pass
    defaults = {"strict":{"t1_entry_score":8.0,"t1_rs_threshold":2.0,"max_positions":4},
                "base":  {"t1_entry_score":7.5,"t1_rs_threshold":1.5,"max_positions":6},
                "loose": {"t1_entry_score":5.0,"t1_rs_threshold":0.8,"max_positions":6}}
...
def save_portfolio(port, acct="base"):
    json.dump(port, open(_paths(acct)["portfolio"], "w", encoding="utf-8"), indent=2, ensure_ascii=False)

def log_trade(record, acct="base"):
    with open(_paths(acct)["trades"], "a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=False) + "\n")

def log_evolution(record, acct="base"):
    with open(_paths(acct)["evolution"], "a", encoding="utf-8") as f:
        f.write(json.dumps(record, ensure_ascii=False) + "\n")
...
# ── 出场 ────────────────────────────────────────────────────
def check_exits(sym, cur, hi=None, lo=None, acct="base", stocks=None):
    portfolio = load_portfolio(acct)
    pos  = portfolio["positions"].get(sym)
    if not pos:
        return None
    ...
    # 板块联动止损收紧
    stop_price = pos.get("stop", cur)
    if stocks and pos.get("cost") and pos.get("stop"):
        try:
            from agents.sector_leadership import get_adjusted_stop_multiplier
            multiplier = get_adjusted_stop_multiplier(sym, stocks)
            if multiplier < 1.0:
                orig_dist = pos["cost"] - pos["stop"]
                if orig_dist > 0:
                    stop_price = round(pos["cost"] - (orig_dist * multiplier), 2)
        except Exception:
            pass
    ...
    if check_lo <= stop_price:
        pnl, proceeds = close_position("STOP_LOSS", cur)
        msgs.append(f"[{sym}] 🛑 止损 ${cur}  {pnl:+.1f}%" + (f" (板块龙头破位，触发收紧止损)" if stop_price > pos.get("stop", 0) else ""))
