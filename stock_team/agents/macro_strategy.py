...
def _load_paid_api_key() -> str | None:
    key = os.environ.get("POLYGON_API_KEY") or os.environ.get("MASSIVE_API_KEY")
    if key:
        return key
    for path in [Path(BASE) / ".env", Path(BASE).parent / ".env"]:
        if not path.exists():
            continue
        try:
            for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
                if "=" not in line or line.strip().startswith("#"):
                    continue
                k, v = line.split("=", 1)
                if k.strip() in ("POLYGON_API_KEY", "MASSIVE_API_KEY"):
...
def _compute_market_indicators(hist: pd.DataFrame, analyst_target=None,
                               price_override=None, data_source="unknown") -> dict:
    ...
    price = float(price_override or closes[-1])
...
def fetch_polygon_daily_aggs(sym: str, api_key: str, days: int = 220) -> list[dict]:
    end = _date.today()
    ...
    url = (
        f"https://api.polygon.io/v2/aggs/ticker/{sym.upper()}/range/1/day/"
        f"{start}/{end}?{params}"
    )
...
def fetch_polygon_snapshot(sym: str, api_key: str) -> dict:
    params = urllib.parse.urlencode({"tickers": sym.upper(), "apiKey": api_key})
    ...
    minute = t.get("min") or {}
    return {
        "price": last.get("p") or minute.get("c") or day.get("c") or prev.get("c"),
        "prev_close": prev.get("c"),
    }


def load_analyst_target(sym: str) -> float | None:
    fund = _load(os.path.join(FIND_DIR, f"fundamentals_{sym.upper()}.json"))
    target = fund.get("target_price")
    return float(target) if target else None


def fetch_market_data_polygon(sym: str, period: str = "6mo") -> dict:
    api_key = _load_paid_api_key()
    if not api_key:
        return {}
    aggs = fetch_polygon_daily_aggs(sym, api_key)
    if len(aggs) < 21:
        return {}
    rows = [{
        "Open":   x["o"],
        ...
        "Close":  x["c"],
        "Volume": x.get("v", 0),
    } for x in aggs if all(k in x for k in ("o", "h", "l", "c"))]
    hist = pd.DataFrame(rows)
    snapshot = fetch_polygon_snapshot(sym, api_key)
    return _compute_market_indicators(
        hist,
        analyst_target=load_analyst_target(sym),
        price_override=snapshot.get("price"),
        data_source="polygon",
    )


def fetch_market_data_yfinance(sym: str, period: str = "6mo") -> dict:
...
def fetch_market_data(sym: str, period: str = "6mo") -> dict:
    try:
        paid = fetch_market_data_polygon(sym, period)
        if paid:
            return paid
    except Exception as e:
        print(f"[macro_strategy] paid data failed for {sym}: {str(e)[:80]} — fallback yfinance")
    return fetch_market_data_yfinance(sym, period)
