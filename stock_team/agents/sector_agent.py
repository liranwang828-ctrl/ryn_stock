...
COMPANY_OVERRIDES = {
    ...
    "RGTI": {
        "etf": "XLK", "etf_name": "科技ETF",
        "name": "量子计算板块",
        "tags": ["量子处理器(QPU)", "量子云服务(QCaaS)", "超导量子芯片"],
        "peers": ["IONQ","QBTS","QUBT","IBM","MSFT","GOOG"],
        "note": "超导量子计算先驱，营收暴增近200%但目前仍亏损，波动极大"
    },
}
...
def analyze(symbol):
    ...
    for _attempt in range(3):
        try:
            ...
            if 'SSL' in err_str or 'TLS' in err_str or 'curl' in err_str.lower():
                # Avoid using emojis to prevent UnicodeEncodeError in GBK terminals
                print(f"  [SSL Error] sector_agent SSL error (attempt {_attempt+1}/3): {err_str[:80]}")
                _time.sleep(2 * (_attempt + 1))
                t = yf.Ticker(symbol)
                continue
            break

    # 缓存降级：若 yfinance info 抓取为空，尝试从本地 company_profile 缓存加载
    if not info:
        cache_path = os.path.join(FINDINGS_DIR, f"company_profile_{symbol.upper()}.json")
        if os.path.exists(cache_path):
            try:
                with open(cache_path, encoding="utf-8") as f:
                    cache_data = json.load(f)
                    info = {
                        "sector": cache_data.get("sector"),
                        "industry": cache_data.get("industry")
                    }
            except Exception:
                pass
    ...
    if not sector_config:
        return (
            ...
            [],
            {},
            [f"该标的板块 {sector_name} 未在系统注册，无法提供相对强弱分析"],
            {
                "judgment": "板块地位中性：个股相对板块暂无可用数据",
                "boundary": "等待个股板块配置补全",
                "anticipated_challenge": "补全该个股的板块同行数据以启用相对强弱评级"
            }
        )
...
def main():
    ...
    if args.round is None:
        ...
        json.dump(msg, open(os.path.join(FINDINGS_DIR, "SectorAgent.json"), "w", encoding="utf-8"), indent=2)
    else:
        msg = compute_phase2_response("SectorAgent", args.symbol, board,
                                      signal, conf, points, refs, _data)
        if not msg:
            json.dump({}, open(os.path.join(FINDINGS_DIR, f"SectorAgent_r{args.round}.json"), "w", encoding="utf-8"))
            return
        msg["sector_info"] = extra
        json.dump(msg, open(os.path.join(FINDINGS_DIR, f"SectorAgent_r{args.round}.json"), "w", encoding="utf-8"), indent=2)
