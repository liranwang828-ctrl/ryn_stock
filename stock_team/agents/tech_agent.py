...
def main():
    ...
    if args.round is None:
        ...
        json.dump(msg, open(os.path.join(FINDINGS_DIR, "TechAgent.json"), "w", encoding="utf-8"), indent=2)
    else:
        msg = compute_phase2_response("TechAgent", args.symbol, board,
                                      signal, conf, points, refs, _data)
        if not msg:
            json.dump({}, open(os.path.join(FINDINGS_DIR, f"TechAgent_r{args.round}.json"), "w", encoding="utf-8"))
            return
        json.dump(msg, open(os.path.join(FINDINGS_DIR, f"TechAgent_r{args.round}.json"), "w", encoding="utf-8"), indent=2)
