...
# 并行隔离：每个 CIO 进程可设置 STOCK_CIO_RUNDIR，使用独立工作目录互不冲突
_CIO_RUNDIR = os.environ.get("STOCK_CIO_RUNDIR", "").strip()
PERSONA_SYNTH_PATH = os.path.join(BASE, "persona_synthesis.json")

if _CIO_RUNDIR:
    BOARD_PATH         = os.path.join(_CIO_RUNDIR, "discussion_board.jsonl")
    VERIFIED_PATH      = os.path.join(_CIO_RUNDIR, "data_verified.json")
    STRATEGY_PATH      = os.path.join(_CIO_RUNDIR, "strategy_result.json")
    FINDINGS_DIR       = os.path.join(_CIO_RUNDIR, "findings")
    LOCK_PATH          = os.path.join(_CIO_RUNDIR, "run.lock")
    PERSONA_SYNTH_PATH = os.path.join(_CIO_RUNDIR, "persona_synthesis.json")
...
def find_persona(agent_name, persona_dir=None):
    ...
    # Check new layout: personas/*/rules.json
    for path in _glob.glob(os.path.join(persona_dir, "*", "rules.json")):
        try:
            ...
            continue
    # Check old layout: personas/*.json
    for path in _glob.glob(os.path.join(persona_dir, "*.json")):
        try:
            ...
            continue
    # Check archive: personas/archive/*.json
    for path in _glob.glob(os.path.join(persona_dir, "archive", "*.json")):
        try:
            p = _json.load(open(path, encoding="utf-8"))
            if p.get("assigned_to") == agent_name:
                return p
...
def apply_persona(msg, data, agent_name, board=None, persona_dir=None):
    import json as _json
    from stock_team.agents.persona_engine import PersonaEngine
