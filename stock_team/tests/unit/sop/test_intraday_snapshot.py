...
def test_plan_vs_now_dist_formulas():
    """距离字段分母统一用 price"""
    from stock_team.data_ingest.intraday_snapshot import compute_dist_fields
    price = 102.0
...
def test_thesis_signal_breach_on_stop():
    ...
    from stock_team.data_ingest.intraday_snapshot import compute_thesis_signal
...
def test_entry_go_expired_after_10_30():
    ...
    from stock_team.data_ingest.intraday_snapshot import compute_entry_go_status
...
def test_build_plan_vs_now_fields():
    ...
    from stock_team.data_ingest.intraday_snapshot import build_plan_vs_now
    summary = {
        "entry": {"entry_base": 100.0, "stop_loss": 95.0, "target_price": 110.0,
...
def test_detect_event_hard_stop_breach():
    ...
    from stock_team.data_ingest.intraday_snapshot import detect_event
    event = detect_event(
...
def test_append_and_read_snapshot(tmp_path):
    ...
    from stock_team.data_ingest.intraday_snapshot import append_snapshot, read_latest_snapshot
    date = "2026-05-20"
...
def test_update_poll_state_merges_new_keys(tmp_path):
    ...
    from stock_team.data_ingest.intraday_snapshot import update_poll_state
