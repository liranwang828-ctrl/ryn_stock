...
def test_load_thresholds_returns_defaults_when_no_file():
    """无配置文件时返回内置默认值"""
    from stock_team.core.premarket_decision import load_thresholds
    with patch("stock_team.core.premarket_decision._THRESHOLDS_PATH", "/nonexistent/path.json"):
        t = load_thresholds()
...
def test_load_thresholds_reads_file(tmp_path):
    ...
    with patch("stock_team.core.premarket_decision._THRESHOLDS_PATH", str(p)):
        t = load_thresholds()
...
def test_hard_veto_thesis_broken():
    from stock_team.core.premarket_decision import _check_hard_vetoes
...
def test_soft_veto_ma20_high():
    from stock_team.core.premarket_decision import _check_soft_vetoes
...
def test_soft_veto_stage2_insufficient():
    ...
    with patch("stock_team.core.premarket_decision._is_stage2_insufficient", return_value=True):
        result = _check_soft_vetoes(
            sym="LITE",
            watchlist_result={"ma20_dev": 0.01, "rs5": -1.0, "narrative_hit": True},
            macro={"yield_curve": "正常", "dxy": 99, "macro_score": 6},
...
def test_evaluate_entry_ke_ruchang():
    """0个硬否决 + 0个软否决 → 可入场"""
    from stock_team.core.premarket_decision import evaluate_entry
    with patch("stock_team.core.premarket_decision._THRESHOLDS_PATH", "/nonexistent"), \
         patch("stock_team.core.premarket_decision._is_stage2_insufficient", return_value=False):
