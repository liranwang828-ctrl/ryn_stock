...
from stock_team.core.entry_guard import parse_trade_input
...
from stock_team.core.entry_guard import generate_entry_card
...
def test_card_contains_symbol():
    """卡片应包含股票代码"""
    with patch("stock_team.core.entry_guard.get_plan", _mock_get_plan):
...
def test_card_no_plan_fallback():
    """无当日计划时应返回简版卡片并提示"""
    with patch("stock_team.core.entry_guard.get_plan", lambda sym, date_str=None: None):
...
from stock_team.core.entry_guard import record_trade
...
def _read_positions(tmpdir):
    with open(os.path.join(tmpdir, "config", "positions.json"), encoding="utf-8") as f:
        return json.load(f)
...
def test_record_buy_new_position():
    ...
    try:
        with patch("stock_team.core.entry_guard.BASE", tmpdir), \
             patch("stock_team.core.entry_guard.get_plan", _mock_get_plan):
...
from stock_team.core.entry_guard import handle_trade_text
...
def test_handle_trade_text_no_plan_still_records():
    ...
    try:
        with patch("stock_team.core.entry_guard.BASE", tmpdir), \
             patch("stock_team.core.entry_guard.get_plan", lambda sym, date_str=None: None):
