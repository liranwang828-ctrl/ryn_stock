...
def tmp_state(monkeypatch, tmp_path):
    monkeypatch.setattr("stock_team.core.session_manager.BASE", str(tmp_path))
    yield tmp_path

from stock_team.core.session_manager import advance_node, get_status, NODE_NAMES
...
def test_add_symbol():
    from stock_team.core.session_manager import add_symbol
    add_symbol("MRVL")
    add_symbol("AAOI")
    status = get_status()
...
from stock_team.core.session_manager import push_alert, drain_alerts
...
def test_push_writes_discussion_board(tmp_state):
    ...
    line = json.loads(open(board, encoding="utf-8").readline())
    assert line["type"] == "signal"
    assert line["symbol"] == "MRVL"
    assert "time" in line

from stock_team.core.session_manager import handle_text
from unittest.mock import patch

def test_handle_text_trade_buy():
    with patch("stock_team.core.session_manager.handle_trade_text",
...
from stock_team.core.session_manager import run_node
...
def test_run_node1_calls_premarket(tmp_state):
    ...
    with patch("stock_team.core.session_manager.subprocess") as mock_sub:
...
def test_run_node_updates_state(tmp_state):
    with patch("stock_team.core.session_manager.subprocess"):
