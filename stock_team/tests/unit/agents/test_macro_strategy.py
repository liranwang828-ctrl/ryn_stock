...
def test_fetch_market_data_prefers_paid_polygon_when_available(monkeypatch):
    from agents.macro_strategy import fetch_market_data
    hist = _make_price_history()
    aggs = []
    ...
    with patch("agents.macro_strategy.fetch_polygon_daily_aggs", return_value=aggs), \
         patch("agents.macro_strategy.fetch_polygon_snapshot", return_value={"price": 250.0, "prev_close": 240.0}), \
         patch("agents.macro_strategy.load_analyst_target", return_value=300.0), \
         patch("agents.macro_strategy.fetch_market_data_yfinance") as yf_fallback:
        data = fetch_market_data("TEST")
    yf_fallback.assert_not_called()
    assert data["data_source"] == "polygon"
    assert data["price"] == 250.0
    assert data["analyst_target"] == 300.0


def test_fetch_market_data_falls_back_to_yfinance_when_paid_source_fails(monkeypatch):
    from agents.macro_strategy import fetch_market_data
    monkeypatch.setenv("POLYGON_API_KEY", "test-key")
    fallback = {"price": 101.0, "ma20": 100.0, "ma50": 99.0, "ma200": 98.0,
                "atr14": 2.0, "atr21": 2.1, "fib_382": 95.0, "fib_618": 93.0,
                "swing_high_90d": 110.0, "swing_low_90d": 80.0,
                "swing_low_14d": 96.0, "rsi14": 55.0, "volume_ratio": 1.0,
                "analyst_target": 120.0, "sentiment_score": None}
    with patch("agents.macro_strategy.fetch_polygon_daily_aggs", side_effect=RuntimeError("paid down")), \
         patch("agents.macro_strategy.fetch_market_data_yfinance", return_value=fallback) as yf_fallback:
        data = fetch_market_data("TEST")
    yf_fallback.assert_called_once_with("TEST", "6mo")
...
def test_build_nodes_integration():
    ...
    import os, json
    mkt  = _default_mkt()
    memo = json.load(open(os.path.join(next(p for p in [os.path.abspath(__file__)] + [os.path.dirname(os.path.abspath(__file__))] + [os.path.dirname(os.path.dirname(os.path.abspath(__file__)))] + [os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))] + [os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))] if os.path.exists(os.path.join(p, "templates"))),
