...
| 加方法学 | 自动提示运行 `python scripts/causal_verification_test.py` 进行时序锁未来函数泄漏检验，并与主线分支核对 |
| 启动近期回测 | 自动提示并引导运行 `python scripts/walk_forward_analysis.py` 进行滚动窗口向前行走 (Walk-Forward) 拼接测试 |
| 新建仓位 X / 加 X / 建仓 X / 我要买 X / 买了 X / 打算买 X / 想建 X / 入场 X | invoke position-builder skill |
| 组合健康 / 持仓检查 / 整体风险 / 周报 / 风险怎么样 / 整体看一下 / 组合风险 / 持仓状态 | portfolio-health skill |
