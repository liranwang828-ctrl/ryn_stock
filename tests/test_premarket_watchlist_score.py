...
def test_rank_watchlist_sorted_by_score():
    ...
    from agents.premarket_watchlist_score import rank_watchlist
    macro  = {"yield_curve": "正常", "dxy": 100, "cpi_latest": "2.5%", "macro_score": 6}
    regime = {"overnight_signal": "RKLB+6%", "regime": "trending_up"}

    with patch("agents.premarket_watchlist_score.score_stock") as mock_score:
...
def test_interactive_select_default(monkeypatch):
    """直接回车确认时返回推荐列表"""
    from agents.premarket_watchlist_score import interactive_select
