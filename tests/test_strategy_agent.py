...
def test_strategy_excludes_veto_agent():
    ...
    for m in board:
        ...
        new_board.append(m)
    with open(f"{BASE}/discussion_board.jsonl", "w", encoding="utf-8") as f:
        for m in new_board:
...
def test_strategy_handles_null_stop_loss():
    ...
    board = [json.loads(l) for l in open(f"{FIXTURE}/discussion_board_phase1.jsonl", encoding="utf-8") if l.strip()]
    for m in board:
